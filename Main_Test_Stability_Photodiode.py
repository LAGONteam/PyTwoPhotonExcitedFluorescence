...
class Timer_Measure(QObject):
    ...
    def run(self):
        ...
        for wavelength in self.list_of_wavelength:
            ...
            self.power_meter_P100USB(wavelength)
            print("Wavelength of the power_meter PM100USB:", power_meter_USB.sense.correction.wavelength)
    ...
    def power_meter_P100USB(self, wavelength):
        print("Current value    :", power_meter_USB.read)
        print("Wavelength       :", power_meter_USB.sense.correction.wavelength)

        #print(power_meter_USB.sense.correction._get_the_doc())
        power_meter_USB.sense.correction.wavelength=int(wavelength)
        print("Wavelength       :", power_meter_USB.sense.correction.wavelength)
        print("Power range limit:", power_meter_USB.sense.power.dc.range.upper)
...
class Main(QWidget):
    ...
    def __init__(self):
        ...
        self.list_measuring_power_thermal = []
        ...
        if str(status) == "b'1'":
            laser.Chameleon().closeShutterBlocking()
            print("Laser closed")
        print(power_meter_USB.read)
        a = power_meter_USB.getconfigure
        print(a)
    ...
    def save_data(self, data_x, data_y, data_y2, data_y3, wavelength, theta):
        """
        ...
        file.touch()
        i=0
        head_data = f"time (s) \t Photodiode current (mV) \t Powermeter thermal_PM100A (mW) \t Powermeter sensor_PM100USB (W)\n"
        with open(file, "w") as w:
            w.write(head_data)
        for n in data_x:
            data = f"{data_x[i]} \t {data_y[i]} \t {data_y2[i]} \t {data_y3[i]} \n"
    ...
    def save_figure(self, x, y, y2, y3, wavelength, theta):
        """
    ...
    def end_of_measure(self):
        ...
        self.save_data(data_x=self.list_measuring_time, data_y=self.list_measuring_power, data_y2=self.list_measuring_power_thermal, data_y3=self.list_measuring_power_sensor, wavelength=self.wavelength,
                       theta=self.theta)
        self.save_figure(x=self.list_measuring_time, y=self.list_measuring_power, y2=self.list_measuring_power_thermal, y3=self.list_measuring_power_sensor, wavelength=self.wavelength,
                         theta=self.theta)
        self.list_measuring_power.clear()
        self.list_measuring_time.clear()
        self.list_measuring_power_sensor.clear()
        self.list_measuring_power_thermal.clear()
    ...
    def measure(self):
        ...
        #void, y_thermal = self.read_PMA100()
        y_sensor = power_meter_USB.read
        print(y_sensor)
        #y_thermal_corr = y_thermal*1000000
        y_sensor_corr = y_sensor * 1# 1000000
        #y_sensor_corr = round(y_sensor_corr, 1)
        #y_thermal_corr = round(y_thermal_corr, 1)
        self.plot(x=self.delay, y=None, y2= None, y3=y_sensor_corr)
    ...
    def define_wavelength(self):
        ...
        for n in range(abs(temp_value)):
            if self.wavelength_start>self.wavelength_end:
                print("self.waveelngth_step", self.wavelength_step)
    ...
    def plot(self, x, y, y2, y3):
        ...
        """
        print("Plot")
        self.list_measuring_power_thermal.append(0)
        self.list_measuring_power_sensor.append(y3)
        self.list_measuring_power.append(0)
        self.list_measuring_time.append(x)
        self.live_power.setLabel("left", units="power / W")
        self.live_power.setLabel("bottom", units="Time / s")
        #self.live_power.plot(self.list_measuring_time, self.list_measuring_power, pen="r", symbol="o", symbolPen="r", symbolBrush=0.5, show=True)
        #self.live_power.plot(self.list_measuring_time, self.list_measuring_power_thermal, pen="y", symbol="o", symbolPen="y",  symbolBrush=0.5, show=True)
        self.live_power.plot(self.list_measuring_time, self.list_measuring_power_sensor, pen="w", symbol="o", symbolPen="w",
